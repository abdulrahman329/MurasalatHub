...
<nav x-data="{ open: false }" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 px-4 py-4">
    <div class="flex justify-between items-center">
        <!-- Left: Toggle + Logo -->
        <div class="flex items-center space-x-4">
            <button data-drawer-target="logo-sidebar" data-drawer-toggle="logo-sidebar" class="text-gray-500 dark:text-gray-400 focus:outline-none sm:hidden">
                <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 20 20">
                    <path d="M3 5h14M3 10h14M3 15h14" />
                </svg>
            </button>
            <a href="{{ route('dashboard') }}" class="text-lg font-semibold text-gray-900 dark:text-white">MyApp</a>
        </div>
        ...
        <div class="relative" x-data="{ open: false }">
            ...
            <!-- Dropdown menu -->
            <div x-show="open" @click.away="open = false" x-transition class="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-700 rounded shadow z-50">
                <div class="block px-4 py-2 text-sm text-gray-600 dark:text-gray-300">Manage Account</div>
                <x-dropdown-link href="{{ route('profile.show') }}" >{{ __('Profile') }}</x-dropdown-link>
                @if (Laravel\Jetstream\Jetstream::hasApiFeatures())
                    <x-dropdown-link href="{{ route('api-tokens.index') }}">{{ __('API Tokens') }}</x-dropdown-link>
                @endif
                <div class="border-t border-gray-200 text-gray-500 dark:border-gray-700"></div>
